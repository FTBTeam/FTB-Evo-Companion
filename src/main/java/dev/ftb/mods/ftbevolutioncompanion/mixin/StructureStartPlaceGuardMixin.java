...
public abstract class StructureStartPlaceGuardMixin {
    ...
    private void ftbevolutioncompanion$guardPlaceInChunk(WorldGenLevel level, StructureManager structureManager, ChunkGenerator generator, RandomSource random, BoundingBox box, ChunkPos chunkPos, Operation<Void> original) {
        try {
            original.call(level, structureManager, generator, random, box, chunkPos);
        } catch (Throwable t) {
            StructureStart self = (StructureStart) (Object) this;
            FTBEVOLUTIONCOMPANION$LOGGER.error(
